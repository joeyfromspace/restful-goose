  ...
  function insertEmbeddedRelationshipsIntoAttributes(attributes, relationships) {
    ...
    _.forEach(relationships, function(rel, key) {
      ...
      if (Array.isArray(rel.data)) {
        prop[key] = _.map(rel.data, 'id');
      } else {
        prop[key] = rel.data.id;
      }
  ...
  function toJSONAPIObject(doc) {
    ...
    _.forEach(Model.schema.paths, function(p, name) {
      ...
      if (Array.isArray(attribs[name])) {
        obj.relationships[name] = { data: _.map(attribs[name], function(rel) {
          return { type: toResourceType(p.options.ref), id: rel };
        })};
      } else {
        obj.relationships[name] = { data: { type: toResourceType(p.options.ref), id: attribs[name] }};  
      }
