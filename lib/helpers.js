...
module.exports = (function() {
   ...
   */
  var digestQuery = function(query, Model) {
    var q = {};
    var f = 'filter[simple]';
    ...
    var _parseKey = function(key) {
      var o = {};
      var arr = key.split('][');
      var schemaPath,operation,pathName,value;
      arr.shift();
      // Get the operation from the end and omit the brackets
      arr[arr.length - 1] = arr[arr.length - 1].substring(0, arr[arr.length - 1].length - 1);
      pathName = _.camelCase(arr[0]);
      operation = _.defaultTo(arr[1], null);
      value = query[key];
      schemaPath = Model.schema.path(pathName);
  ...
  var querySearch = function(req, res, next) {
    ...
    extQuery = digestQuery(req.query, res.rg.Model);
