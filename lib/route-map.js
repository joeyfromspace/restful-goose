...
var RouteMap = (function() {
  ...
  RouteMap.prototype.createModel = function(req, res, next) {
    function _createRelationship(done) {
      ...
      var update = Array.isArray(req.data) ? _.map(req.data, 'id') : [req.data.id];
  ...
  RouteMap.prototype.updateModel = function(req, res, next) {
    function _updateRelationship(done) {
      ...
      debug(req.data);
      var data = Array.isArray(req.data) ? _.map(req.data, 'id') : req.data.id;
  ...
  RouteMap.prototype.setupResponse = function(req, res, next) {
    ...
    res.data = { data: ((!Array.isArray(res.model)) || (Array.isArray(res.model) && res.model.length)) ? helpers.serialize(res.model, req.params.relationship_type, res.relRef) : ((res.model === []) ? [] : null )};
