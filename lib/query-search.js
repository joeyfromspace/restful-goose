var async = require('async');
var _ = require('lodash');

module.exports = function(mongoose, Model, query, callback) {
  function buildSearchObject() {
    var ignoreKeys = ['limit' ,'skip', 'sort', 'per_page', 'page'];
    var obj = _.pickBy(query, function(v, k) {
      return ignoreKeys.indexOf(k) === -1;
    });
    _.forOwn(obj, function(value, key) {
      if (!Model.schema.paths[key]) {
        obj[key] = value;
      } else if (Model.schema.paths[key].options.type.schemaName === 'String' || Model.schema.paths[key].options.type === String) {
        obj[key] = { $regex: value, $options: 'i' };    
      } else {
        obj[key] = value;
      }
    });

    return obj;
  }
  function buildSortObject() {
    var obj = {};
    var sorts = query.sort.split(',');
    _.forEach(sorts, function(s) {
      if (s.charAt(0) === '-') {
        obj[s.substr(1)] = -1;
      } else {
        obj[s] = 1;
      }
    });

    return obj;
  }
  function buildQueryOptions() {
    var obj = {};
    obj.limit = parseInt(query.limit, 10) || parseInt(query.per_page, 10) || false;
    obj.skip = parseInt(query.skip, 10) || ((parseInt(query.page, 10) * (obj.limit || 25)) - (obj.limit || 25)) || 0;
    obj.sort = query.sort ? buildSortObject() : undefined;

    return obj;
  }
  var opts = query ? buildQueryOptions() : null;
  var search = buildSearchObject();

  var getCount = function(done) {
    Model.where(search).count(done);
  };

  var getQuery = function(count, done) {
    Model.find(search, null, opts, function(err, results) {
      var resultsObject;

      if (err) {
        return done(err);
      }

      var pageMeta = {
        per_page: opts.limit,
        page: query.page,
        total_pages: Math.ceil(count / (opts.limit))
      };

      resultsObject = {
        data: results,
        meta: {
          limit: opts.limit,
          skip: opts.skip,
          count: count
        }
      };

      if (query.page || query.per_page) {
        Object.assign(resultsObject.meta, pageMeta);
      }

      return done(null, resultsObject);
    });

  };

  async.waterfall([getCount, getQuery], callback);
};